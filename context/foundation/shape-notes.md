...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: pain category
      decision: decision paralysis — when to increase / hold / deload
    - topic: insight
      decision: apps log workouts but leave the "what next?" decision to the user
    - topic: primary persona scope
      decision: self-directed lifters (individuals across many gyms)
    - topic: auth strategy
      decision: login required; flat user model; own plans and history only
    - topic: mvp scope
      decision: "login → AI plan → edit → log session → progression suggestion; 3 weeks"
    - topic: business logic depth
      decision: "inputs: completed sets with load+reps; output: increase/hold/deload + suggested load; shown right after logging"

    - topic: product framing
      decision: "web-app; small scale; deadline 2026-09-13; after-hours only"
    - topic: non-goals
      decision: "no advanced periodization; no multi-format import; no sharing; no devices; no mobile"
  frs_drafted: 6
  quality_check_status: accepted
---

# Shape Notes — ProgressApp

Seed (from `idea-notes.md`):

> Manualne planowanie progresji obciążeń w treningu siłowym wymaga doświadczenia i regularnego liczenia — początkujący i średniozaawansowani nie wiedzą, kiedy zwiększyć ciężar, a kiedy zrobić deload, co prowadzi do stagnacji lub przetrenowania.
...
### MVP first-session flow
1. User signs up / logs in
2. User picks a goal and generates a plan via AI
3. User reviews and edits the plan / exercises
4. User logs a training session
5. User sees an automatic progression suggestion (increase / hold / deload)
6. User accepts or adjusts the suggestion before the next workout
...
- Session and training-history data remain visible only to the account owner (from Guardrails; no additional NFRs specified for MVP).

## Open Questions

1. **Additional NFRs** — none beyond privacy from Guardrails; confirm if perceived latency / browser support matter for MVP.
...
All required elements present (Access Control, Business Logic one-sentence rule, artifacts, timeline ≤ 3 weeks, Non-Goals). Supporting business-logic detail and additional NFRs remain in Open Questions by explicit choice; quality_check_status: accepted.
