...
ProgressApp already ships Astro 6 SSR with `@astrojs/cloudflare` ^13.5, `wrangler` ^4.90, and a correct Workers Static Assets config (`main: "@astrojs/cloudflare/entrypoints/server"`). Interview constraints — request/response only, global users, external Supabase + OpenRouter, Cloudflare familiarity — all align with Workers. It scored Pass on all five agent-friendly criteria; Vercel is the runner-up if a JAMstack commercial host is preferred later.

## Platform Comparison

Scored Pass / Partial / Fail against CLI-first, managed/serverless, agent-readable docs, stable deploy API, and MCP/integration. Soft weights: global edge preferred; existing Cloudflare familiarity breaks ties; co-located DB not required (Supabase + OpenRouter stay external). No hard filter dropped candidates (persistent connections not required).
...
**Cloudflare** — Native stack path; Wrangler deploy/rollback/tail; `llms.txt` + MDX docs; managed MCP; ~$5/mo Paid Workers recommended for SSR CPU. Global edge. External Supabase/OpenRouter fit the fetch model.
...
Won on stack fit (already wired), global edge, agent ops (Wrangler + docs + MCP), and low MVP cost with Paid Workers. External Supabase/OpenRouter match the interview answer. Tech-stack frontmatter hinted `cloudflare-pages`; current platform direction is **Workers + Static Assets** — this project’s `wrangler.jsonc` already follows that path.
...
- **Secrets**: Local `.dev.vars` (gitignored); production via `npx wrangler secret put SUPABASE_URL` / `SUPABASE_KEY` (and later OpenRouter keys). Mirror the same names in GitHub Actions secrets for CI builds. Rotate by putting a new secret value, then redeploy if the Worker caches env at deploy time.
...
| Node-compat / dependency break on workerd | Devil's advocate / Research | M | H | Keep `nodejs_compat`; smoke-test auth + OpenRouter paths after dependency bumps; prefer edge-safe SDKs |
...
| Edge ≠ global DB/AI latency | Devil's advocate | H | L | Accept for MVP; keep OpenRouter calls short; cache plan templates if needed later |
...
2. Local secrets: copy `.env.example` values into `.dev.vars` as `SUPABASE_URL` / `SUPABASE_KEY` (and OpenRouter when added). Use `npm run dev` — Astro 6 already runs the Workers (`workerd`) runtime; do not assume a separate Pages dev command.
